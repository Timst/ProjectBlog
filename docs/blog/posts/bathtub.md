...
I live in Seattle, which in meteorological terms is in the [Köppen Csb](https://en.wikipedia.org/wiki/K%C3%B6ppen_climate_classification) area, or “Warm-Summer Mediterranean Climate” zone, the same as Los Angeles or French Provence. It’s also zone 9a in the [USDA Plant Hardiness system](https://planthardiness.ars.usda.gov/), which is solidly in the warmer half of the scale, and should allow me to grow pretty much anything that’s not tropical directly in the soil.

Here’s the problem though: in reality, this place is essentially Waterworld nine months out of twelve, if you’re lucky. It never stops raining, the sun is just a distant memory, and we even get snow every now and then (not enough to brighten things up, but definitely enough to murder your garden). So growing an orange tree here? Outside? I don’t think so.
...
That lamp and the heater are behind a [Philips Hue smart plug](https://www.philips-hue.com/en-us/p/hue-smart-plug/046677552343) that’s set up to run from 07:00 to 21:00 (plants need darkness to rest, and a temperature differential to flower). Hue is typically a smart light brand, and I do indeed have a dozen or so Hue lamps around the house; I had this plug to control some string lights which I’m no longer using. It works and it’s easy to control, so I’m using that for now.

Moving to the bathroom also opened up the exciting possibility of automated irrigation.

I dipped my toe in the water (jokes) by building a thing to automatically fill up the humidifier tank:

<figure>
  <img src="/bathtub/humidifier.webp" alt="Humidifier setup">
  <figcaption>Note that this is a different humidifier than on the last pic: I swapped my cheapo device for a <a href="https://www.amazon.com/dp/B0C4GHYH2M">Levoit smart humidifier</a>, which can be controlled remotely, which might or might not come in handy later.</figcaption>
</figure>
...
## So did this work or
I originally wrote this article immediately after finishing up the system back in 2024, but I'm now updating it in 2026. So first, let's see how I felt back then:

Things started well! I rapidly started getting some flowers:
...
I also set up a camera to take a pic every hour and do a timelapse, using a [PiCamera](https://www.amazon.com/dp/B01ER2SKFS). I also ended up getting a third tree (a lime tree).

## No but really, how did it go

Right, so: not as well as I'd have hoped. Past the initial flowering, the trees didn't develop all that much. The orange and lime trees got some good leaf growth in, and the occasional flower, but it would fail to develop into a viable fruit.

I'm not entirely sure what the issue was. I would add fertilizer manually, and it might not have been quite enough. I don't think the root systems were pot-bound, because when the summer came, I moved them outside, and they did end up making flowers and even a couple of fruits. Then I forgot to bring them back in for the winter and they died. I KNEW this would happen!!

## What could have been
Among improvements I was considering for the system, it would have been nice to have a more fine-grained control of the heat and humidity. I was relying on the onboard thermostat/humidistat of the heater and humidifier, and it made for a pretty unstable atmosphere:

<figure>
  <img src="/bathtub/heat.webp" alt="Chart of temperature and humidity for the room">
</figure>

The plants probably didn’t mind too much, but it would be nice to be able to control that stuff directly. Also the humidifier wasn't on the smart plug circuit, so I wasn't able to emergency stop it, which was quite the oversight as it was literally in the bathtub and powered by a 110V cord, thereby making it win the coveted “Most Likely To Start A Fire” award.

Generally speaking though, even though this wasn't going to be passing any inspection, I don’t think it was too dangerous. I expect a humidifier to be pretty well shielded against humidity, and the grow lamp had IP67 waterproofing. The parts that work with water were contained in a bathtub, so flooding was unlikely (and indeed, never happened), especially with my leak detector (which also emits a super loud alarm, so it’s not just used for my jury-rigged emergency stop system). Also, since it’s a bathroom, all the outlets are [GFCI](https://en.wikipedia.org/wiki/Residual-current_device)-protected, so there’s that.

The sketchiest part was probably the space heater (always a great source of domestic fires) and the mess of power strips and cables. I wasn't worried about overloading them (I had a Kill-A-Watt on the entire system and it peaked at 1000W, well within what they could handle), but they’re meant for indoor use, American sockets are notoriously bad, and the room did get humid. Outdoor-rated plugs and cables would probably have been a good investment.

Speaking of the Kill-A-Watt, this setup consumed a total of 459kWh over the four-ish months I ran it, of which 70% was the heater, another 26% was the lamp, and the rest was the humidifier and Pi. At my electricity rates, that added up to about $55. No prize for calculating how many oranges I could have bought for that money (especially given my incredible yield of zero (0) oranges). For what it’s worth, the power grid in Seattle is 90% renewables, so hopefully it’s not too big of a crime against nature.

## Conclusion
So it didn't really work, but it was fun. Don't let that stop you though! Really, the message here is that growing fruit trees in your shower is not only possible, it’s necessary. Everybody should do it. A chicken, for every pot, and a citrus tree in every bathtub, I say!